...
const _: [(); 16] = [(); std::mem::size_of::<StatxTimestamp>()];
const _: [(); 256] = [(); std::mem::size_of::<Statx>()];

fn stat_timestamp(ts: [u64; 2]) -> StatxTimestamp {
    StatxTimestamp {
        tv_sec: ts[0] as i64,
        tv_nsec: ts[1] as u32,
        __reserved: 0,
...
fn stat_to_statx(st: &stat, _requested_mask: u32) -> Statx {
    Statx {
        ...
        stx_atime: stat_timestamp(st.st_atim),
        stx_btime: StatxTimestamp::default(),
        stx_ctime: stat_timestamp(st.st_ctim),
        stx_mtime: stat_timestamp(st.st_mtim),
        stx_rdev_major: 0,
        stx_rdev_minor: 0,
        stx_dev_major: 0,
...
pub extern "C" fn statx_handler(
    ...
    let statat_flags = (arg3 as i32) & !AT_STATX_SYNC_TYPE;
    let mut statbuf = stat::default();
    let ret = imfs::with_imfs(|state| {
        state.statat(arg1cage, arg1 as i32, &pathname, &mut statbuf, statat_flags)
    });
    ...
    let statxbuf = stat_to_statx(&statbuf, arg4 as u32);
    let this_cage = getcageid();
    let _ = copy_data_between_cages(
        this_cage,
        arg5cage,
        &statxbuf as *const Statx as u64,
        this_cage,
        arg5,
        arg5cage,
        std::mem::size_of::<Statx>() as u64,
        0,
    );
