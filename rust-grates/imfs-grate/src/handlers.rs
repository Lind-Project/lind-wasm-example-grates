...
pub extern "C" fn pipe_handler(
    ...
    let (readfd, writefd) = match imfs::with_imfs(|state| state.pipe(cage_id, false)) {
        Ok(pair) => pair,
        Err(e) => return e,
    };

    let this_cage = getcageid();
    let pipefds = [readfd as i32, writefd as i32];
    match copy_data_between_cages(
        this_cage,
        arg1cage,
        pipefds.as_ptr() as u64,
        this_cage,
        arg1,
        arg1cage,
        std::mem::size_of_val(&pipefds) as u64,
        0,
    ) {
        Ok(_) => 0,
        Err(_) => -14,
...
pub extern "C" fn pipe2_handler(
    ...
    let flags = arg2 as i32;
    if flags & !grate_rs::constants::fs::O_CLOEXEC != 0 {
        return -22;
    }

    let cage_id = arg1cage;
    let cloexec = (flags & grate_rs::constants::fs::O_CLOEXEC) != 0;
    let (readfd, writefd) = match imfs::with_imfs(|state| state.pipe(cage_id, cloexec)) {
        Ok(pair) => pair,
        Err(e) => return e,
    };
