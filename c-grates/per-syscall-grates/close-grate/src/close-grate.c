...
int close_grate(uint64_t cageid, uint64_t arg1, uint64_t arg1cage,
  ...
  return make_threei_call(CLOSE_SYSCALL, 0, cageid, arg1cage, arg1,
...
int main(int argc, char *argv[]) {
  ...
  if (pid < 0) {
    ...
    uint64_t fn_ptr_addr = (uint64_t)(uintptr_t)&close_grate;
    int ret = register_handler(cageid, CLOSE_SYSCALL, grateid, fn_ptr_addr);
  ...
  int status;
  while (wait(&status) > 0) {
    printf("[Grate|close] terminated, status: %d\n", status);
  }
