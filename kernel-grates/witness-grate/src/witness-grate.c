...
#include <threei.h>        /* current 3i ABI */

#include "ed25519.h"

/* current-ABI syscall numbers (were SYS_* in lind) */
#define WNR_READ    __NR_read
#define WNR_WRITE   __NR_write
#define WNR_EXECVE  __NR_execve
#define WNR_OPENAT  __NR_openat
...
/* ============================================================
 * Shared forwarding  (current 3i: 4-arg make_threei_call)
 * ============================================================ */

static long forward_syscall(uint32_t syscallno, pid_t cageid,
                            const int arg_cage[6], unsigned long args[6]) {
    return make_threei_call((uint32_t)syscallno, cageid, arg_cage, args);
}
...
static int sign_record_detached(const syscall_record_t *rec,
                                ...
                                uint8_t sig[SIG_LEN]) {
    ed25519_sign(sig, (const unsigned char *)rec, sizeof(*rec), pubkey, privkey);
    return 0;
}
...
/* ============================================================
 * Signed forwarding  (packed args[6] + arg_cage[6])
 * ============================================================ */

static long signed_forward(uint32_t syscallno, pid_t cageid,
                           const int arg_cage[6], unsigned long args[6]) {
    witness_ctx_t *ctx = get_or_create_witness_ctx((uint64_t)cageid);
    ...
    rec.cageid = (uint64_t)cageid;
    for (i = 0; i < 6; i++) {
        rec.args[i] = (uint64_t)args[i];
        rec.arg_cages[i] = (uint64_t)arg_cage[i];
    }
    ...
    return forward_syscall(syscallno, cageid, arg_cage, args);
}
...
long execve_witness(pid_t cageid, const int arg_cage[6], unsigned long args[6]) {
    return signed_forward(WNR_EXECVE, cageid, arg_cage, args);
}

long read_witness(pid_t cageid, const int arg_cage[6], unsigned long args[6]) {
    return signed_forward(WNR_READ, cageid, arg_cage, args);
}

long write_witness(pid_t cageid, const int arg_cage[6], unsigned long args[6]) {
    return signed_forward(WNR_WRITE, cageid, arg_cage, args);
}

long openat_witness(pid_t cageid, const int arg_cage[6], unsigned long args[6]) {
    return signed_forward(WNR_OPENAT, cageid, arg_cage, args);
}
...
static void register_witness_handlers(int cageid, int grateid) {
    unsigned long execve_ptr = (unsigned long)(uintptr_t)&execve_witness;
    unsigned long openat_ptr = (unsigned long)(uintptr_t)&openat_witness;
    unsigned long read_ptr   = (unsigned long)(uintptr_t)&read_witness;
    unsigned long write_ptr  = (unsigned long)(uintptr_t)&write_witness;
    int ret;

    ret = register_handler(cageid, WNR_EXECVE, grateid, execve_ptr);
    if (ret != 0) {
        fprintf(stderr, "[WitnessGrate] register execve failed ret=%d\n", ret);
        assert(0);
    }
    ret = register_handler(cageid, WNR_OPENAT, grateid, openat_ptr);
    if (ret != 0) {
        fprintf(stderr, "[WitnessGrate] register openat failed ret=%d\n", ret);
        assert(0);
    }
    ret = register_handler(cageid, WNR_READ, grateid, read_ptr);
    if (ret != 0) {
        fprintf(stderr, "[WitnessGrate] register read failed ret=%d\n", ret);
        assert(0);
    }
    ret = register_handler(cageid, WNR_WRITE, grateid, write_ptr);
    if (ret != 0) {
...
int main(int argc, char *argv[]) {
    ...
    /* PARENT (grate) only from here. */
    {
        int cageid = pid;
        debug_printf("[WitnessGrate] Parent preparing child cage=%d grate=%d\n",
                     cageid, grateid);
...

    {
        ...
        while ((w = wait(&status)) > 0) {
            ...
            snprintf(wl, sizeof(wl),
                     "WAIT reaped pid=%d raw_status=0x%x exited=%d code=%d signaled=%d",
                     (int)w, status, WIFEXITED(status),
    ...
    debug_printf("[WitnessGrate] done exit=%d\n", last_exit);
