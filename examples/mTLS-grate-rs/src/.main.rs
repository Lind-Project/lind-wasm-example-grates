...
static NEXT_SESSION_ID: Mutex<u64> = Mutex::new(1);
...
static TLS_SESSIONS: Mutex<Option<HashMap<u64, TlsStream>>> = Mutex::new(None);
...
impl Read for ThreeiSocket {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        let ret = make_threei_call(
            ...
            self.fd_owner_cage,
            self.fd_owner_cage,
            self.real_fd, self.fd_owner_cage,
            buf.as_mut_ptr() as u64, this_cage,
            buf.len() as u64, this_cage,
            0, 0, 0, 0, 0, 0, 0,
        );
        match ret {
...
fn main() {
    ...
    *TLS_SESSIONS.lock().unwrap() = Some(HashMap::new());
