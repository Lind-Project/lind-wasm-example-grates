...
build_c_grate() {
    local dir="$1"
    local grate_dir="$REPO_ROOT/${gtype}-grates/$dir"
...
for gi in "${!G_NAMES[@]}"; do
    ...
    if [[ ! -d "$REPO_ROOT/${gtype}-grates/$gdir" ]]; then
        continue
    fi
    ...
    for ti in "${!T_GRATE_IDX[@]}"; do
        ...
        tsrc="${T_SRC[$ti]}"
        test_src_path="$REPO_ROOT/${gtype}-grates/$gdir/$tsrc"
        ...
        if [[ -n "$tfiles" ]]; then
            while IFS= read -r f; do
                [[ -z "$f" ]] && continue
                src="$REPO_ROOT/${gtype}-grates/$gdir/$f"
                if [[ -f "$src" ]]; then
                    cp "$src" "$LINDFS/"
                fi
